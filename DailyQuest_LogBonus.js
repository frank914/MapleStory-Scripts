...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		if ( 前提log獎勵[0] ) {
			msg += "完成" + 前提log獎勵[1][0] + "可領取以下額外獎勵\r\n";
			if (前提log獎勵[2].length > 0) {
				msg += "獎勵物品: \r\n"
				for (var i = 0; i < 前提log獎勵[2].length; i++) {
					var id = 前提log獎勵[2][i][0];
					var qty = 前提log獎勵[2][i][1];
					var hour = 前提log獎勵[2][i][2];
					msg += "\t#v" + id + ":##t" + id + ":# " + qty + "個";
					msg += "\r\n";
				}
			}
        ...
        if (rewardItem.length > 0) {
            if (!cm.hasAllSpace(rewardItem.length)||(前提log獎勵[0]&&!cm.hasAllSpace(rewardItem.length+前提log獎勵[2].length)) ) {
		...
		if ( 前提log獎勵[0] ) {
			...
			if (前提log獎勵[4] > 0) {
				msg += "獎勵GASH: " + 前提log獎勵[4] + "\r\n"
				cm.getPlayer().modifyCSPoints(1, 前提log獎勵[4]);
			}

			if (前提log獎勵[5] > 0) {
				msg += "獎勵楓點: " + 前提log獎勵[5] + "\r\n"
				cm.getPlayer().modifyCSPoints(1, 前提log獎勵[5]);
			}
			if (前提log獎勵[6] > 0) {
				msg += "獎勵紅利: " + 前提log獎勵[6] + "\r\n"
				gainTTPoint(前提log獎勵[6]);
			}
		}
        cm.sendNext(msg);
