...
function action(mode, type, selection) {
    ...
    if (status == 0) {
	var auto = "每日任務代號";
        currentQuest = cm.getCacheLog(auto);
        if (currentQuest == null) {
			cm.addDailyCacheLog(auto, getRandomInt(0,quests.length-1),cm.getPlayer().getWorld()); 
			currentQuest = cm.getCacheLog(auto);
		}
		if ( cm.getPlayer().getAccLogValue("全服每日任務刷新") < 0 ) {
            for (var i = 0; i < quests[currentQuest][8].length; i++) {
                var mobid = quests[currentQuest][8][i][0];
				cm.getPlayer().removeMobKilledCount(mobid);
			}
			cm.getPlayer().setAccDailyLogValue("全服每日任務刷新", 1); 
		}
        if (currentQuest == -1) {
